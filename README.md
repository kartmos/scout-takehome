...
**Greenhouse map** — an optional Konva-canvas floor plan, opened via a bottom-right launcher button. The map renders a 40 × 40 m world with photo markers drawn from real `x`/`y` coordinates stored in the database, displayed over a subdued greenhouse floor-plan background at 60% opacity. The drawer has three states: **hidden** (compact launcher only), **compact** (a fixed ~380 × 280 px panel), and **expanded** (a centred, nearly full-screen modal dialog with Escape/focus-return support). Zoom (1×–6×) and pan are supported; marker size stays constant on screen across zoom levels. In **compact mode**, clicking a marker or the map canvas immediately filters the gallery to photos within a **3 m radius** of that location. In **expanded mode**, clicks set a draft location only — a persistent action bar shows a preview count and requires pressing **Apply location** to commit the filter; Cancel/Escape/Return discards the draft without changing the gallery. The selected location is shown as a chip with a **Clear location** button; normal cursor pagination resumes when the filter is cleared. A coordinate readout beside the zoom controls shows the draft location in expanded mode and the applied location in compact mode. A single bounded query of up to 200 matching photos backs the map; if the catalog has more, a disclosure is shown. The map query is skipped until the drawer is first opened.
