...
| `017` | `READY` | Production runtime and documentation | Multi-stage API/web images, Nginx/reverse proxy, healthchecks, CPU/RAM/cache-volume limits; internal/public S3 endpoints; external-S3 production topology; clean-clone README, seed/test/run steps, architecture/cache/trade-offs | Docker уже установлен на 006 |
...
Backend через 012.3 и обязательный frontend через 015.1 закрыты. Промт `017-production-runtime-and-documentation` создан и имеет статус `READY`; этап 016 остаётся optional bonus map.
