...
| `021` | `READY FOR REVIEW` | Caddy/GHCR VPS deployment | `.github/workflows/deploy.yml` (workflow_run+CI-poll SHA gate, `PRODUCTION_DEPLOY_ENABLED` kill switch, `workflow_dispatch` exact-SHA/tag recovery path, multi-platform GHCR build/publish, SSH deploy); `deploy/Caddyfile` (pinned `caddy:2.9.1-alpine`, automatic HTTPS, admin API loopback-only, proxies only to `web:8080`); `deploy/compose.server.yaml` (Caddy-only public ports, private api/web, bounded resources, seed profile); `deploy/.env.production.example`; `deploy/deploy.sh` + `deploy/rollback.sh` (validate/pull/switch/health-check/auto-restore, immutable-tag rollback); README "Deploy to a VPS" section; `deploystepbystep.md` full Russian operator guide; `.gitignore` entries for the real server env file | `caddy validate`/`caddy fmt` pass; `docker manifest inspect caddy:2.9.1-alpine` confirms linux/amd64+linux/arm64; `docker compose -f deploy/compose.server.yaml config` renders with placeholder env (Caddy-only ports, no MinIO, immutable `image:tag`, required vars fail fast when missing); `bash -n` and `shellcheck` clean on both scripts; `actionlint`+PyYAML parse `deploy.yml` clean; `docker compose config` (local) and `docker compose -f compose.production.yaml config` (placeholder env) unaffected; `git diff --check` clean. Real GHCR publication, DNS, SSH, ACME, seed, and production smoke remain pending user infrastructure (not exercised) |
