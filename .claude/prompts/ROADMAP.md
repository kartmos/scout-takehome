...
| `009-seed-client-and-data-api-smoke` | `READY` | Bounded rerunnable public-API seed client and one MinIO ingest→read smoke | Экономный промт: focused unit suite и один обязательный integration smoke |

Complete Data API через этап 008 закрыт. Текущий следующий этап — `009-seed-client-and-data-api-smoke`.
...
| `009` | `READY` | Seed client and Data API smoke | Идемпотентный seed через публичный upload-link API с concurrency default `2`, строгая сверка dataset, exit codes; ingest→list/get end-to-end smoke через MinIO и SQLite | Нет |
...
Обычные implementation-этапы используют focused gate; полный gate выполняется
после нескольких связанных промтов. Этап считается выполненным после применимого
для него уровня проверки:
...
3. На каждом этапе прошли formatter, focused tests, затронутый build/type check и `git diff --check`.
4. Не осталось executable, coverage, temp DB или других build artifacts.
5. Targeted race/repetition/integration запускаются только на соответствующей границе риска, а не автоматически в каждом промте.
6. Полные backend gate и review выполняются на `012`; полный frontend gate и review — на `015`; runtime gate — на `017`; clean-clone final gate — на `018`.
7. Обязательные по ТЗ тесты bbox, thumbnail parsing/math, key reducer/component и ingest→read smoke не удаляются.
...
```text
/clear
/implement-task @.claude/prompts/009-seed-client-and-data-api-smoke.md
```
